...
### Destructuring
Destructing allows binding using pattern matching, with support for matching arrays and objects.  Destructuring is fail-soft, similar to standard object lookup `foo["bar"]`, producing `undefined` values when not found.
...
Iteration is based on these duck-typed interfaces (using [TypeScript](#typescriptlang.org) type syntax for exposition only):
```TypeScript
...
The generator interface is (using [TypeScript](#typescriptlang.org) type syntax for exposition only):
